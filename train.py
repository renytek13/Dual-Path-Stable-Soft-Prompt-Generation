...
def reset_cfg(cfg, args):
    ...
    if args.early_epoch:
        cfg.EARLY = args.early_epoch
    
    if args.late_epoch:
        cfg.LATE = args.late_epoch
...
def extend_cfg(cfg):
    ...
    elif 'DPSPG' in args.trainer:
        cfg.TRAINER.DPSPG = CN()
        cfg.TRAINER.DPSPG.PREC = "fp16"                 # fp16, fp32, amp  
        cfg.TRAINER.DPSPG.N_CTX = 16                    # number of text context vectors
        cfg.TRAINER.DPSPG.CSC = False                   # class-specific context
        cfg.TRAINER.DPSPG.CLASS_TOKEN_POSITION = "end"  # 'middle' or 'end' or 'front'
        cfg.TRAINER.DPSPG.POS_CTX_INIT = ""             # positive initialization words
        cfg.TRAINER.DPSPG.NEG_CTX_INIT = ""             # negativa initialization words
        if 'TRANSFORMER' in args.trainer:
            cfg.TRAINER.DPSPG.PREC = "fp32"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--trainer", type=str, default="DPSPG_TRANSFORMER", help="name of trainer")
    ...
    parser.add_argument("--alpha", type=float, default=0.2, help="neg loss weight alpha")
    parser.add_argument("--early_epoch", type=int, default=50, help="early stop train")
    parser.add_argument("--late_epoch", type=int, default=0, help="late start evaluation")
