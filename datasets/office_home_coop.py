...
def split_data(cfg, image_dir):
    ...
    # domains.remove('dpspg_coop_splits')
    domains.sort()
    print(f'OfficeHome domains: {domains}, use {cfg.DATASET.TARGET_DOMAINS} to obtain best prompt as label.')
    
    domain_dir = os.path.join(image_dir, cfg.DATASET.TARGET_DOMAINS[0])
    class_names = listdir_nohidden(os.path.join(domain_dir, 'train'))
    class_names.sort()
    print('OfficeHome has {} categories: {}'.format(len(class_names), class_names))
    
    p_trn = 0.7
    print(f"Splitting into {p_trn:.0%} train and {1 - p_trn:.0%} val")

    train, val = [], []
    for type_dir in ['train', 'val']:
        for label, class_name in enumerate(class_names):
            class_path = os.path.join(domain_dir, type_dir, class_name)
...
class OfficeHome_CoOp(DatasetBase):

    dataset_dir = "office_home_dg"

    def __init__(self, cfg, custom=False):
        root = os.path.abspath(os.path.expanduser(cfg.DATASET.ROOT))
        self.dataset_dir = os.path.join(root, self.dataset_dir)
        self.image_dir = os.path.join(self.dataset_dir, "images")

        self.split_dir = os.path.join(self.dataset_dir, "dpspg_coop_splits")
        ...
        if custom:
            ...
            if os.path.exists(self.split_path):
                ...
                train, val, test = split_data(cfg, self.dataset_dir)
                save_split(train, val, test, root, self.split_path)
        else:
            train = self.read_data(self.dataset_dir, cfg.DATASET.TARGET_DOMAINS, "train")
            val = self.read_data(self.dataset_dir, cfg.DATASET.TARGET_DOMAINS, "val")
            test = self.read_data(self.dataset_dir, cfg.DATASET.TARGET_DOMAINS, "val")
