...
def split_data(cfg, image_dir):
    ...
    domains.sort()
    print(f'TerraIncognita domains: {domains}, use {cfg.DATASET.TARGET_DOMAINS} to obtain best prompt as label.')
    
    domain_dir = os.path.join(image_dir, cfg.DATASET.TARGET_DOMAINS[0])
    class_names = listdir_nohidden(domain_dir)
    class_names.sort()
    print('TerraIncognita has {} categories: {}'.format(len(class_names), class_names))
...
class TerraIncognita_CoOp(DatasetBase):
